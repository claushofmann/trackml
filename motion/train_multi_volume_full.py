...
from motion.multi_volume_tracker import FullMultiVolumeTracker
...
if __name__ == '__main__':
    ...
    if config_to_use.sequencing == 'annoy':
        train_data = ld.DerivedRangeDetectorFiles(create_from=train_data, derive_task=ld.create_annoy_sequences(config_to_use.bucket_parameter))
    ...
    if config_to_use.sequencing == 'theta':
        train_data = ld.DerivedRangeDetectorFiles(create_from=train_data, derive_task=ld.create_sequences(config_to_use.bucket_parameter))
    ...
    test_data = ld.RootRangeDetectorFiles(start_range=1400, end_range=1432)
    if config_to_use.synthetic:
        test_data = ld.DerivedRangeDetectorFiles(create_from=test_data, derive_task=ld.create_synthetic(7000))
    if config_to_use.sequencing == 'annoy':
        test_data = ld.DerivedRangeDetectorFiles(create_from=train_data,
    ...
    estimator = FullMultiVolumeTracker(config_to_use, do_load_predict=True)
